Intelligent Multi-Agent Campus GuideAn advanced, full-stack web application designed to assist students and visitors in navigating the college campus, discovering events, and getting instant support via an intelligent AI chatbot.🚀
 ...
 🔒 Secure AuthenticationRole-Based Access Control (RBAC): Distinct portals for Students (User), Administrators (Admin), and Visitors (Guest).
 Guest Mode: Allows visitors to use navigation features without creating an account.
 🛠️ Tech StackFrontend: React.js, react-map-gl, maplibre-glBackend: Python FastAPI, MYSQL, PydanticDatabase: MySQLExternal 
 APIs:Mapbox: For maps and routing.Brevo (formerly Sendinblue): For transactional emails.
 Open-Meteo: For weather data.Wikipedia: For general knowledge.
 ⚙️ Installation & Setup1. PrerequisitesNode.js & npm installed.Python 3.10+ installed.MySQL Server running.API Keys for Mapbox and Brevo.
 2. Database SetupLog in to your MySQL shell.Create the database:CREATE DATABASE campus_guide_db;
...
3. Backend Setup (FastAPI)cd backend
